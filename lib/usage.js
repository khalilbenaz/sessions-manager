...
const { getAgyQuota } = require('./agents');
...
module.exports = function (ctx) {
  ...
  route('GET', /^\/api\/usage$/, async ({ res }) => {
    ...
    let agyQuota = null;
    try { agyQuota = await getAgyQuota(); } catch { }
    json(res, 200, { ...agg, perDay, top: top.slice(0, 15), agyQuota, note: 'Coût estimé indicatif pour les modèles Claude & Antigravity.' });
  });
